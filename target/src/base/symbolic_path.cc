...
void SymbolicPath::Serialize(string* s) const {
  ...
  //fianl remove
  /*
  fprintf(stderr," branches size = %u\n", len);
  fprintf(stderr," branches size = %u\n", len_ll);
  */
  //s->append((char*)&len, 8);
 ...
 // s->append((char*)&len, sizeof(len));
  ...
  len_ll = (long long) len;
//  s->append((char*)&len, sizeof(len));
  // s->append((char*)&len, 8);
  s->append((char*)&len_ll, 8);
  //final remove 
  /*
  fprintf(stderr," const size = %u\n", len);
  */
//  s->append((char*)&constraints_idx_.front(), constraints_.size() * sizeof(size_t));
  // s->append((char*)&constraints_idx_.front(), constraints_.size() * 4);
//  long long constraints_idx_front_ll = (long long) constraints_idx_.front();
 // fprintf(stderr," const idx_front1 = %u\n", constraints_idx_.front());
 // fprintf(stderr," const idx_front2 = %u\n", constraints_idx_front_ll);
  // s->append((char*)&constraints_idx_.front(), constraints_.size() * 8);
//  s->append((char*)&constraints_idx_front_ll, constraints_.size() * 8);
//  s->append((char*)&constraints_idx_front_ll, constraints_.size() * 8);

  for(size_t i = 0 ; i < len; i++) {
	long long tmp = constraints_idx_[i];
	s->append((char*)&tmp, 8);
  }
