...
bool Z3Solver::Solve(const map<var_t,type_t>& vars,
  ...
  { // Constraints.
    for (PredIt i = constraints.begin(); i != constraints.end(); ++i) {
      ...
      assert(terms.size() > 0);
      context& ctx = terms[0].ctx();
      array<Z3_ast> _args(terms);
      Z3_ast r = Z3_mk_add(ctx, _args.size(), _args.ptr());
      ctx.check_error();
      expr terms_expr = expr(ctx,r);
      // expr pred(ctx);
      switch((*i)->op()) {
