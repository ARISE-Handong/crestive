...
void Search::RemoteRun(const vector<value_t>& inputs) {
  //make input in string form
  // send input
  // send_input();
  // fprintf(stderr, "remote run 0\n");
  string input_string="";
  if (inputs.size()!=0) {
    for(int i = 0 ; i < inputs.size(); i++) {
      input_string += std::to_string(inputs[i]) + "\n";
    }
  }
  // else {
  //   input_string = "empty";
  ...
  send_input(input_string.c_str());
  auto middle = std::chrono::high_resolution_clock::now();
  // receive szd_execution
  // recv_szd_execution();

//  fprintf(stderr, "before recv_szd_execution\n");
  recv_szd_execution();
 // fprintf(stderr, "after recv_szd_execution\n");
  auto end = std::chrono::high_resolution_clock::now();
  elapsed_time_program_ += (end - start);
  elapsed_time_send_input_ += (middle-start);
  elapsed_time_recv_szd_execution_ += (end - middle);
}
...
bool Search::CheckPrediction(const SymbolicExecution& old_ex,
  ...
  // if((new_ex.path().branches().size() <= branch_idx)){
  //   printf("first-2\n");
  //   return false;
  // }
  //
  // else if ((old_ex.path().branches().size() <= branch_idx)) {
  //   printf("first-1\n");
  //   return false;
  // }
...
void BoundedDepthFirstSearch::PureDfs(size_t pos, int depth, SymbolicExecution& prev_ex) {
  ...
  for (size_t i = path.constraints().size()-1; (i < path.constraints().size()) && (i >= pos) && (depth > 0); i--) {
    ...
    // printf("after updatecover\n");

    // Check for prediction failure.
    size_t branch_idx = path.constraints_idx()[i];
    // fprintf(stderr, "path.constraints_idx[%u]=%u\n",i, path.constraints_idx()[i]);
    // printf("after branch_idx\n");


    // printf("prev_ex \n");
    // for (size_t i = 0; i < prev_ex.path().branches().size(); i++) {
      // fprintf(stderr, "%d ", prev_ex.path().branches()[i]);
    // }
    // fprintf(stderr, "\n");

    // printf("cur_ex \n");
    // for (size_t i = 0; i < cur_ex.path().branches().size(); i++) {
      // fprintf(stderr, "%d ", cur_ex.path().branches()[i]);
    // }
    // fprintf(stderr, "\n");
...
void BoundedDepthFirstSearch::ReversedDfs(size_t pos, int depth, SymbolicExecution& prev_ex) {
  ...
  for (size_t i = pos; (i < path.constraints().size()) && (depth > 0); i++) {
    // Solve constraints[0..i].
    // printf("before SolveAtBranch\n");
    if (!SolveAtBranch(prev_ex, i, &input)) {
      continue;
    }
    // Run on those constraints.
    RunProgram(input, &cur_ex);
    // printf("before updatecoverage\n");
    UpdateCoverage(cur_ex);
    // printf("after updatecoverage\n");
    // Check for prediction failure.
    size_t branch_idx = path.constraints_idx()[i];
    // printf("before checkprediction\n");
    if (!CheckPrediction(prev_ex, cur_ex, branch_idx)) {
      fprintf(stderr, "Prediction failed!\n");
      continue;
    }
