...
class Search {
 ...
 protected:
  ...
  std::chrono::duration<double> elapsed_time_send_input_;
  std::chrono::duration<double> elapsed_time_recv_szd_execution_;
  ...
  */
  ...
  void WriteCoverageToFileOrDie(const string& file); void LaunchProgram(const vector<value_t>& inputs);
  void RemoteRun(const vector<value_t>& inputs);
};
