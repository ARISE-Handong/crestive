...
bool SymbolicPath::Parse(istream& s) {
  ...
  // fprintf(stderr, "costraint size: %d\n",len);
  // fprintf(stderr, "costraint size1: %d\n",constraints_.size());
  // fprintf(stderr, "costraint siz2e: %d\n",constraints_idx_.size());
  //printf("before read2\n");
  //s.read((char*)&constraints_idx_.front(), len * 4);
  //printf("after read2\n");
  ...
  for(size_t i = 0 ; i < len; i++ ) {
    size_t tmp;
    s.read((char*)&tmp, sizeof(size_t));
    constraints_idx_[i] = tmp;
    // fprintf(stderr, "constraints_idx[%u]:%u\n",i, constraints_idx_[i]);
  }

  // fprintf(stderr, "costraint idx front: %d\n",constraints_idx_.front());
  for (ConIt i = constraints_.begin(); i != constraints_.end(); ++i) {
    *i = new SymbolicPred();
    if (!(*i)->Parse(s))
      return false;
